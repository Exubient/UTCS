...
class Assignment5Tests(unittest.TestCase):
    ...
    def call_methods(self, td, key):
        td[key] = 1
        td[key]
        td.get(key)
    
    def test_construct_empty(self):
        self.call_methods(assignment5.TreeDict(), "1")
        
    def test_construct_unordered_pairs(self):
        self.call_methods(assignment5.TreeDict((i, str(i)) for i in shuffled_list), 1)
        
    def test_construct_ordered_pairs(self):
        self.call_methods(assignment5.TreeDict((i, str(i)) for i in range(1, 101)), 2)
        
    def test_construct_dict(self):
        self.call_methods(assignment5.TreeDict({1: "1", 2: "2"}), 2)
        self.call_methods(assignment5.TreeDict({"1": "1", "2": "2"}), "1")
