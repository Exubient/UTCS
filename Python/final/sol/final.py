...
class RandomWriter(object):
    ...
    def generate(self):
        ...
        node = self.model.state_dict[random.choice([key for key in self.model.state_dict.keys()])]
    ...
    def save_text(self, filename_or_file_object):
        ...
        for key in self.model.state_dict:
            outputStr = str(key) + ":"
            targetStr = ""
            if len(self.model.state_dict[key].next_states) == 0:
                fi.write(outputStr)
                fi.write("\n")
                continue     # skipped the ones without followers
            for ele in self.model.state_dict[key].next_states:
    ...
    def load_pickle(cls, filename_or_file_object):
        ...
        rw = RandomWriter()
        rw.model.state_dict = pickle.load(fi)
        fi.close()
        return rw
