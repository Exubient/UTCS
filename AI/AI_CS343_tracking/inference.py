...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        JailCell = self.getJailPosition()

        # give true value to the jailcell
        if noisyDistance == None:
            allPossible[JailCell] = True
        # get manhattan distance/sonar distance and store to allPossible
        else:
            for posIndex in self.legalPositions:
                distance = util.manhattanDistance(posIndex, pacmanPosition)
                allPossible[posIndex] = self.beliefs[posIndex] * emissionModel[distance]
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPosition in self.legalPositions:
            ...
            for newPosition, probability in newPosDist.items():
                beliefProbability= probability * self.beliefs[oldPosition]
                allPossible[newPosition] += beliefProbability
...
class ParticleFilter(InferenceModule):
    ...
    def __init__(self, ghostAgent, numParticles=300):
        ...
        "set the number of particles at the first time"
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        self.particles = []  #create a empty list

        for i in range(self.numParticles):  #iterate for number of particles
            evenLocation = i % len(self.legalPositions)     #use '%' opreate to make sure particles should be evenly(uniformly)
            self.particles.append(self.legalPositions[evenLocation])
    ...
    def observe(self, observation, gameState):
        ...
        """
        noisyDistance = observation
        emissionModel = busters.getObservationDistribution(noisyDistance)   # get Weight
        pacmanPosition = gameState.getPacmanPosition()
        "*** YOUR CODE HERE ***"

        if noisyDistance == None:   # When a ghost is capture by Pacman
            self.particles = []
            for i in range(self.numParticles):
                self.particles.append(self.getJailPosition())
        else:
            isZero = True       # Check if it is zero weights or not
    ...
    def elapseTime(self, gameState):
        ...
        # Obtain new distances, positions
        for particle in self.particles:
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        positions = []
        for tup in itertools.product(*[self.legalPositions for i in range(self.numGhosts)]):
            positions.append(tup)

        random.shuffle(positions)  # Must shuffle the positions

        
        self.particles = [positions[i % len(positions)] for i in range(self.numParticles)]
        ...
        for particle in self.particles:
            ...
            for i in range(self.numGhosts):
                if noisyDistances[i] != None:
                    ...
                    weight *= emissionModels[i][trueDistance]   # update weights
            weights[particle] += weight # update weights
            if weight > 0: isZero = False
