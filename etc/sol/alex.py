...
parser.add_argument(
    "--input", help="The input file to train on", default=sys.stdin)
parser.add_argument(
    "--output", help="The input file to train on", default=sys.stdout)
...
next_state = self.graph.nodes[
    self.random_pick(path_dict)]
yield self.state.name[-1]
if not next_state:
...
"""
    ...
    filename_or_file_object, 'rb')
with fi:
cls.graph = pickle.load(
    fi)
fi.close()
rw = cls(
    1)
...
windows = self.windowed(
    data.split(), self.level)
else:
windows = self.windowed(
    list(data), self.level)

if self.token == Tokenization.byte:
if not isinstance(data, (int, bytes)):
raise TypeError(
    "Data must be of type int or bytes")
windows = self.windowed(
    bytearray(data), self.level)
...
if self.parent:
self.parent.add_path(
    self.graph.nodes[window])
self.graph.update_node(
    self.parent)
self.parent = self.graph.nodes[
    window]
else:
new_node = Node(
    window)
